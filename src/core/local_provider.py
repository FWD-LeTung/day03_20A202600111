...
class LocalProvider(LLMProvider):
    ...
    def __init__(self, model_path: str, n_ctx: int = 4096, n_threads: Optional[int] = None):
        ...
        if Llama is None:
            raise ImportError(
                "LocalProvider requires `llama-cpp-python`, which failed to import. "
                "On Windows, install Build Tools or switch DEFAULT_PROVIDER to openai/google.\n"
                f"Original error: {type(_LLAMA_IMPORT_ERROR).__name__}: {_LLAMA_IMPORT_ERROR}"
            )
